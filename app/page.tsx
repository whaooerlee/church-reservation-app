...
// 공간 id → 색상 / 이름 매핑은 그대로 두고…
  ...
  const eventContent = (info: any) => {
    ...
    // 🔍 teamName이나 title에 이미 공간명이 들어있으면 중복 제거
    const rawText = teamName || info.event.title;
    const cleanText = rawText.replace(/\[?\s*${spaceName}\s*\]?/g, '').trim();

    // ✅ 최종 표시 문자열
    const mainText = `[${spaceName}] ${cleanText} ${start && end ? `${start}~${end}` : ''}`;

    const wrap = document.createElement('div');
    wrap.textContent = mainText;
    wrap.style.display = 'block';
    wrap.style.fontSize = '0.7rem'; // 한 줄로 보기 좋게 작게
    wrap.style.lineHeight = '1.2';
    wrap.style.whiteSpace = 'nowrap';
    wrap.style.overflow = 'hidden';
    wrap.style.textOverflow = 'ellipsis';
    wrap.style.color = '#0a1e4f';
